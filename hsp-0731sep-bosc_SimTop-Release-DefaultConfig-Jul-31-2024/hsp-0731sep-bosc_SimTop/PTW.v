...
module PTW(
  ...
  wire             pageFault =
    ...
    & ~(level == 2'h2 | _to_find_pte_T & io_mem_resp_bits[18:10] == 9'h0 | ~(|level)
  ...
  wire             _GEN_13 = ~(|level) & ~onlyS2xlate & ~(find_pte | accessFault);
  ...
  wire             _GEN_25 = sent_to_pmp & ~mem_addr_update;
  wire             _GEN_26 = accessFault & ~idle;
  wire             _GEN_27 = guest_fault & ~idle;
  wire             _GEN_28 = _GEN_27 | _GEN_26;
  wire             _GEN_29 = _GEN_27 | _GEN_26 | _GEN_24 | _GEN_22 & onlyS2xlate;
  wire             _GEN_30 = ~_GEN_29 & _GEN_19;
  wire             _GEN_31 = io_mem_resp_valid & ~w_mem_resp;
  wire             _GEN_32 = ~_GEN_31 & (_GEN_28 | s_llptw_req);
  wire             _GEN_33 = _GEN_31 | _GEN_29 | mem_addr_update;
  wire             _GEN_34 = mem_addr_update & _GEN_13;
  wire             _GEN_35 = io_llptw_ready & io_llptw_valid_0;
  wire             _GEN_36 = accessFault | pageFault | ppn_af;
  wire             _GEN_37 = ~mem_addr_update | _GEN_13;
  wire             _GEN_38 = io_resp_valid_0 & _GEN_11;
  wire             _GEN_39 = ~_GEN_14 & _GEN_38;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (_GEN_31)
        ...
        af_level <= _GEN_16;
      s_pmp_check <=
        flush | _GEN_27 | _GEN_26 | _GEN_25
        | (~_GEN_22 | onlyS2xlate | io_hptw_resp_bits_h_resp_gpf
           | io_hptw_resp_bits_h_resp_gaf) & (~_GEN_12 | _GEN_18) & s_pmp_check;
      s_mem_req <=
        flush | (~_GEN_34 | s2xlate) & (perfEvents_4_2 | _GEN_28 | ~_GEN_25 & s_mem_req);
      s_llptw_req <=
        flush
        | (mem_addr_update
             ? _GEN_13 | (io_llptw_valid_0 ? _GEN_35 | _GEN_32 : _GEN_39 | _GEN_32)
             : _GEN_32);
      w_mem_resp <= flush | _GEN_31 | ~perfEvents_4_2 & (_GEN_28 | w_mem_resp);
      s_hptw_req <=
        flush | ~(mem_addr_update & _GEN_13 & s2xlate)
        & (_GEN_27 | _GEN_26 | _GEN_21
           | (_GEN_12 ? ~(_GEN_18 | _GEN_8) & s_hptw_req : ~_GEN_8 & s_hptw_req));
      w_hptw_resp <=
        flush | _GEN_27 | _GEN_26 | _GEN_22 | ~_GEN_21 & (_GEN_10 | w_hptw_resp);
      s_last_hptw_req <=
        flush | (~mem_addr_update | _GEN_13 | io_llptw_valid_0 | ~_GEN_14 | _GEN_36)
        & (_GEN_27 | _GEN_26 | _GEN_23 | s_last_hptw_req);
      w_last_hptw_resp <=
        flush | _GEN_27 | _GEN_26 | _GEN_24 | ~_GEN_23 & w_last_hptw_resp;
      mem_addr_update <=
        ~flush
        & (mem_addr_update
             ? ~_GEN_13
               & (io_llptw_valid_0
                    ? ~_GEN_35 & _GEN_33
                    : _GEN_14 ? _GEN_36 & _GEN_33 : ~_GEN_38 & _GEN_33)
             : _GEN_33);
      idle <=
        flush
        | (_GEN_37 ? _GEN_17 : io_llptw_valid_0 ? _GEN_35 | _GEN_17 : _GEN_39 | _GEN_17);
      if (_GEN_24 | _GEN_22) begin
        hptw_pageFault <= io_hptw_resp_bits_h_resp_gpf;
        hptw_accessFault <= io_hptw_resp_bits_h_resp_gaf;
      end
      else begin
        hptw_pageFault <= ~_GEN_12 & hptw_pageFault;
        hptw_accessFault <= ~_GEN_12 & hptw_accessFault;
      end
      if (_GEN_37)
        last_s2xlate <= _GEN_30;
      else if (io_llptw_valid_0)
        last_s2xlate <= ~(_GEN_35 | _GEN_29) & _GEN_19;
      else if (_GEN_14)
        last_s2xlate <= ~(_GEN_36 | _GEN_29) & _GEN_19;
      else
        last_s2xlate <= _GEN_30;
