...
module VSstatusModule(
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (trapToVS_vsstatus_valid) begin
        reg_SIE <= trapToVS_vsstatus_bits_SIE;
        reg_SPIE <= trapToVS_vsstatus_bits_SPIE;
        reg_SPP <= trapToVS_vsstatus_bits_SPP;
      end
      else if (retFromS_vsstatus_valid) begin
        reg_SIE <= retFromS_vsstatus_bits_SIE;
        reg_SPIE <= retFromS_vsstatus_bits_SPIE;
        reg_SPP <= retFromS_vsstatus_bits_SPP;
      end
      else if (w_wen) begin
        reg_SIE <= w_wdata[1];
        reg_SPIE <= w_wdata[5];
