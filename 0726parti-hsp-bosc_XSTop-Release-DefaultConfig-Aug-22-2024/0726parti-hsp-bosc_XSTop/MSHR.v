...
module MSHR(
  ...
  wire            _a_task_io_tasks_txreq_bits_opcode_T_2 = req_param == 3'h1;
  ...
  wire            _GEN_5 = io_alloc_valid | state_s_cbwrdata;
  wire            _acquire_ts_T = io_tasks_txreq_ready & io_tasks_txreq_valid_0;
  wire            _GEN_6 = ~state_s_reissue | io_alloc_valid;
  wire            _state_s_cbwrdata_T_2 =
    dirResult_meta_state[1] & dirResult_meta_dirty | probeDirty;
  wire            _GEN_7 = _acquire_ts_T & ~state_s_reissue & release_valid2;
  wire            _probe_ts_T = io_tasks_source_b_ready & io_tasks_source_b_valid_0;
  wire            _GEN_8 = io_tasks_mainpipe_ready & mp_grant_valid;
  wire            _GEN_9 = ~io_tasks_mainpipe_ready | mp_grant_valid;
  wire            _GEN_10 =
    ~io_tasks_mainpipe_ready | mp_grant_valid | release_valid1 | ~mp_cbwrdata_valid;
  wire            _GEN_11 = io_resps_sinkC_bits_opcode == 3'h5;
  wire            _GEN_12 =
    io_resps_sinkC_valid & (io_resps_sinkC_bits_opcode == 3'h4 | _GEN_11);
  wire            rxdatIsU_PD = io_resps_rxdat_bits_resp == 3'h6;
  wire            _GEN_13 = io_resps_rxdat_valid & io_resps_rxdat_bits_chiOpcode == 6'h4;
  wire            _GEN_14 = io_resps_rxrsp_bits_chiOpcode == 6'h4;
  wire            _GEN_15 = io_resps_rxrsp_valid & _GEN_14 & ~state_w_grant;
  wire            _GEN_16 = io_resps_rxrsp_bits_chiOpcode == 6'h5;
  wire            _GEN_17 = _GEN_16 | _GEN_14;
  wire            _GEN_18 = io_resps_rxrsp_bits_chiOpcode == 6'h3;
  wire            _GEN_19 = io_resps_rxrsp_valid & _GEN_18;
  wire            _GEN_20 = io_resps_rxrsp_valid & io_resps_rxrsp_bits_chiOpcode == 6'h7;
  wire            _GEN_21 = io_replResp_valid & io_replResp_bits_retry;
  wire            _GEN_22 = io_replResp_valid & ~io_replResp_bits_retry;
  wire            _GEN_23 =
    _GEN_22
      ? io_replResp_bits_meta_dirty
      : io_alloc_valid ? io_alloc_bits_dirResult_meta_dirty : dirResult_meta_dirty;
  wire            _GEN_24 = _GEN_22 & (|io_replResp_bits_meta_state);
  wire            _GEN_25 =
    _GEN_22 & (|io_replResp_bits_meta_state) & io_replResp_bits_meta_clients;
  wire            nestedwb_hit_match =
    req_valid & dirResult_hit & _nestedwb_hit_match_T_1 & _nestedwb_hit_match_T_3;
  wire            _GEN_26 = nestedwb_match & io_nestedwb_b_inv_dirty;
  wire            _GEN_27 = nestedwb_hit_match & io_nestedwb_b_toN;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (_GEN_15)
        gotT <= io_resps_rxrsp_bits_resp == 3'h2;
      else if (_GEN_13)
        gotT <= io_resps_rxdat_bits_resp == 3'h2 | rxdatIsU_PD;
      else
        gotT <= ~io_alloc_valid & gotT;
      gotDirty <=
        ~_GEN_15 & (_GEN_13 ? gotDirty | rxdatIsU_PD : ~io_alloc_valid & gotDirty);
      gotGrantData <= _GEN_13 | ~io_alloc_valid & gotGrantData;
      probeDirty <=
        ~_GEN_26 & (io_resps_sinkC_valid & _GEN_11 | ~io_alloc_valid & probeDirty);
      probeGotN <=
      ...
      req_aliasTask <=
        ~_GEN_27 & (io_alloc_valid ? io_alloc_bits_task_aliasTask : req_aliasTask);
      dirResult_hit <=
        ~_GEN_27 & (io_alloc_valid ? io_alloc_bits_dirResult_hit : dirResult_hit);
      if (_GEN_22) begin
      ...
      dirResult_meta_dirty <=
        ...
        & (nestedwb_match
             ? ~io_nestedwb_b_inv_dirty & (io_nestedwb_c_set_dirty | _GEN_23)
             : _GEN_23);
      ...
      state_s_refill <=
        ~_GEN_21
        & (_GEN_8 | (io_alloc_valid ? io_alloc_bits_state_s_refill : state_s_refill));
      state_s_retry <= ~_GEN_21 & (_GEN_8 | io_alloc_valid | state_s_retry);
      ...
      if (io_resps_rxrsp_valid & _GEN_14)
        state_w_grantfirst <= ~state_w_grant | _GEN_13 | _GEN_4;
      else
        state_w_grantfirst <= _GEN_13 | _GEN_4;
      state_w_grantlast <=
        _GEN_15
        | (_GEN_13
             ? state_w_grantfirst
      ...
      state_w_releaseack <=
        ~_GEN_24
        & (io_resps_rxrsp_valid & (_GEN_16 | _GEN_14 & ~state_w_releaseack)
      ...
      if (_GEN_9) begin
        if (_GEN_7)
          state_s_cbwrdata <= ~_state_s_cbwrdata_T_2;
        else
          state_s_cbwrdata <= _GEN_5;
      end
      else if (release_valid1)
        state_s_cbwrdata <=
          ~(dirResult_meta_state[1] & dirResult_meta_dirty | probeDirty);
      else
        state_s_cbwrdata <=
          mp_cbwrdata_valid | (_GEN_7 ? ~_state_s_cbwrdata_T_2 : _GEN_5);
      state_s_reissue <=
        ~_GEN_20 & (_acquire_ts_T & ~state_s_reissue | io_alloc_valid | state_s_reissue);
      state_s_dct <=
      ...
      if (_GEN_21)
        ...
        backoffTimer <= 5'h0;
      if (io_resps_rxrsp_valid & (_GEN_18 | _GEN_17))
        srcid <= io_resps_rxrsp_bits_srcID;
      else if (io_alloc_valid)
        srcid <= 7'h0;
      if (_GEN_13)
        homenid <= io_resps_rxdat_bits_homeNID;
      if (io_resps_rxrsp_valid & _GEN_17)
      ...
      if (_GEN_19)
        ...
        pcrdtype <= 4'h0;
      gotRetryAck <=
        _GEN_19 | (_acquire_ts_T ? ~_GEN_6 & gotRetryAck : ~io_alloc_valid & gotRetryAck);
      gotPCrdGrant <=
        _GEN_20
        | (_acquire_ts_T ? ~_GEN_6 & gotPCrdGrant : ~io_alloc_valid & gotPCrdGrant);
  ...
  assign io_tasks_txreq_bits_opcode =
    release_valid2
      ...
      : (&req_opcode) & _a_task_io_tasks_txreq_bits_opcode_T_2
          ? 6'hC
  ...
  assign io_tasks_txrsp_bits_tgtID =
    (&req_opcode) & _a_task_io_tasks_txreq_bits_opcode_T_2 ? srcid : homenid;
  ...
  assign io_tasks_source_b_bits_param =
    state_s_pprobe
      ? (req_get & dirResult_hit & _b_task_io_tasks_source_b_bits_param_T_4 ? 2'h1 : 2'h2)
