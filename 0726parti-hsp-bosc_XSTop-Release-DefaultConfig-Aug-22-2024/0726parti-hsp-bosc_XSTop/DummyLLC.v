...
module DummyLLC(
  ...
  wire         isWriteNoSnp_1 = req_opcode == 6'h1D | req_opcode == 6'h1C;
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (_GEN_10)
        wBeatCnt <= 1'(wBeatCnt - 1'h1);
      else
        wBeatCnt <= ~alloc & wBeatCnt;
      if (_GEN)
        rBeatCnt <= 1'(rBeatCnt - 1'h1);
      else
        rBeatCnt <= ~alloc & rBeatCnt;
      if (_GEN_1 | ~_GEN_3)
        snpBeatCnt_0 <= ~alloc & snpBeatCnt_0;
      else
        snpBeatCnt_0 <= 1'(snpBeatCnt_0 - 1'h1);
      if (_GEN_4)
        releaseBeatCnt <= 1'(releaseBeatCnt - 1'h1);
      else
        releaseBeatCnt <= ~alloc & releaseBeatCnt;
      if (_GEN_12)
        refillBeatCnt <= 1'(refillBeatCnt - 1'h1);
      else
        refillBeatCnt <= ~alloc & refillBeatCnt;
  ...
  ReceiverLinkMonitor linkMonitor (
    ...
    .io_out_rx_rsp_bits_opcode
      (s_readreceipt
         ? {1'h0, _rn_0_rx_rsp_bits_resp_T_1 | req_opcode == 6'hD, 2'h0}
           | (req_opcode == 6'h1B | req_opcode == 6'h1D | req_opcode == 6'h1C
                ? 4'h5
                : 4'h0)
         : 4'h8),
    ...
    .io_out_rx_dat_bits_resp
      (req_opcode == 6'h7 ? {snpGotDirty, 2'h2} : {2'h0, ~isReadNoSnp_1}),
    ...
    .io_out_rx_snp_bits_opcode
      ((req_opcode == 6'h7 ? 5'h7 : 5'h0) | {2'h0, req_opcode == 6'h26, 2'h0}
