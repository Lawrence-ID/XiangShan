...
module VSegmentUnit(
    ...
    instMicroOp_uop_exceptionVec_4 <=
      _GEN_1
        ? ~addr_aligned & (instMicroOp_uop_fuType[31] | instMicroOp_uop_fuType[33])
          & canTriggerException
        : ~_GEN_3 & instMicroOp_uop_exceptionVec_4;
    instMicroOp_uop_exceptionVec_5 <=
      _GEN_1
        ...
        : _GEN_45
            ...
            : ~_GEN_3 & instMicroOp_uop_exceptionVec_5;
    instMicroOp_uop_exceptionVec_6 <=
      _GEN_1
        ? ~addr_aligned & ~(instMicroOp_uop_fuType[31] | instMicroOp_uop_fuType[33])
          & canTriggerException
        : ~_GEN_3 & instMicroOp_uop_exceptionVec_6;
  ...
  wire              fieldActiveWirteFinish = io_sbuffer_ready & io_sbuffer_valid_0;
  wire              segmentInactiveFinish =
    (_segmentInactiveFinish_T | _segmentInactiveFinish_T_1) & ~segmentActive;
  wire              _GEN_62 =
    _fieldIdx_T & (_segmentInactiveFinish_T | fieldActiveWirteFinish);
  wire              _GEN_63 = segmentIdx != _maxSegIdx_T;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (_GEN_3) begin
        ...
        else if (segmentInactiveFinish & _GEN_63)
          segmentIdx <= _segmentIdx_T_2[7:0];
        if (_GEN_7 | fieldActiveWirteFinish)
          fieldIdx <= _fieldIdx_T ? 4'h0 : 4'(fieldIdx + 4'h1);
        else if (segmentInactiveFinish)
          fieldIdx <= 4'h0;
      end
      if (_GEN_62 | segmentInactiveFinish)
        segmentOffset <=
          41'(segmentOffset
