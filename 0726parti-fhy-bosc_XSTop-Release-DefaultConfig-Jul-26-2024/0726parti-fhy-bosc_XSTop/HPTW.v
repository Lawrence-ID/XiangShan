...
module HPTW(
  ...
  wire        _GEN = find_pte | accessFault;
  wire        _GEN_0 = io_resp_ready & resp_valid;
  assign finish = _GEN & resp_valid;
  wire        _GEN_1 = sent_to_pmp & ~mem_addr_update;
  wire        _GEN_2 = accessFault & ~idle;
  wire        _GEN_3 = io_mem_req_ready & io_mem_req_valid_0;
  wire        _GEN_4 = io_mem_resp_valid & ~w_mem_resp;
  wire        _GEN_5 = mem_addr_update & ~_GEN;
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      s_pmp_check <= flush | _GEN_2 | _GEN_1 | ~_source_T & s_pmp_check;
      s_mem_req <= flush | ~_GEN_5 & (_GEN_3 | _GEN_2 | ~_GEN_1 & s_mem_req);
      w_mem_resp <= flush | _GEN_4 | ~_GEN_3 & (_GEN_2 | w_mem_resp);
      idle <= flush | _GEN & _GEN_0 | ~_source_T & idle;
      mem_addr_update <=
        ~(flush | mem_addr_update & (~_GEN | _GEN_0))
        & (_GEN_4 | _GEN_2 | mem_addr_update);
    end
  end // always @(posedge, posedge)
  wire        _GEN_6 = sent_to_pmp ? io_pmp_resp_ld | io_pmp_resp_mmio : accessFault;
  always @(posedge clock) begin
    ...
    accessFault <=
      ~flush & (_GEN & resp_valid ? ~(_GEN_0 | _source_T) & _GEN_6 : ~_source_T & _GEN_6);
  end // always @(posedge)
