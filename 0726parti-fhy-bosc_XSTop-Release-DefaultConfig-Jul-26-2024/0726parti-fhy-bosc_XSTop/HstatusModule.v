...
module HstatusModule(
  ...
  always @(posedge clock or posedge reset) begin
    if (reset) begin
      ...
      if (trapToHS_hstatus_valid) begin
        reg_GVA <= trapToHS_hstatus_bits_GVA;
        reg_SPV <= trapToHS_hstatus_bits_SPV;
        reg_SPVP <= trapToHS_hstatus_bits_SPVP;
      end
      else begin
        if (w_wen) begin
          reg_GVA <= w_wdata[6];
        ...
        if (retFromS_hstatus_valid)
          reg_SPV <= retFromS_hstatus_bits_SPV;
        else if (w_wen)
          reg_SPV <= w_wdata[7];
      end
      if (w_wen) begin
